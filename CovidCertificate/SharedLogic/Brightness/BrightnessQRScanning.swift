...
/// Handles the behaviour of the application when barcode scanning is needed.
final class BrightnessQRScanning {
    ...
    private func updateScreenBrightness() {
        if isEnabled {
            if screenBrightness == nil {
                screenBrightness = ScreenBrightness(targetBrightness: 0.8)
            }
        } else {
