...
class CertificateLightSummaryView: UIView {
    ...
    init(isActive: Bool) {
        if isActive {
            activateButton = Button(title: UBLocalized.wallet_certificate_light_detail_deactivate_button, style: .normal(.cc_bund))
        } else {
            activateButton = Button(title: UBLocalized.wallet_certificate_light_detail_activate_button)
        }
