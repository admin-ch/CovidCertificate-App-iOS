...
public enum Luhn {
    ...
    public static func generateLuhnCode() -> String {
        ...
        let checksum = calculateLuhnSum(luhnCode)
    ...
    public static func checkLuhnCode(_ luhnCode: String) -> Bool {
        let sum = calculateLuhnSum(luhnCode)
        let remainder = sum % characterSet.count
        return remainder == 0
    }

    private static func calculateLuhnSum(_ luhnCode: String) -> Int {
        return luhnCode.reversed().map {
            characterSet.firstIndex(of: String($0))!
        }.enumerated()
            .map {
                sum(digits: $1 * ($0 % 2 != 0 ? 2 : 1), withModulo: characterSet.count)
            }
            .reduce(0, +)
    }
