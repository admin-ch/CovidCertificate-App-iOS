...
enum TransformationService {
    static func getLightCertificate(qrCode: String,
                                    completionHandler: @escaping (Result<LightCertificate, CertificateLightCreationError>) -> Void) {
    ...
    static func getPdf(qrCode: String,
        ...
        URLSession.certificatePinned.dataTask(with: request) { data, _, error in
            ...
            guard let pdf = try? JSONDecoder().decode(TransformationPdfResponsePayload.self, from: data) else {
                completionHandler(.failure(.NETWORK_PARSE_ERROR))
                return
            }
